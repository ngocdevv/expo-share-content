...
The extension uses only the public `NSExtensionContext.open` path. Apple does not guarantee that a Share Extension can foreground its containing app. Queue delivery does not depend on auto-open.
